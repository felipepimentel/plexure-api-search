...
from .model import APIDataQualityMetrics
...
class DataValidator:
    ...
    def __init__(self):
        ...
        self.required_fields = {"path", "method", "description", "version"}
        self.valid_methods = {"GET", "POST", "PUT", "DELETE", "PATCH"}

    def validate_api_data(
        self, data: List[Dict], strict: bool = False
    ) -> Tuple[bool, List[str]]:
        """Validate API data.
    ...
    def calculate_quality_metrics(self, data: List[Dict]) -> APIDataQualityMetrics:
        ...
        unique_paths = len({item.get("path") for item in data if "path" in item})
