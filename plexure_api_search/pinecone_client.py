    ...
    def upsert_vectors(self, vectors: List[Dict], batch_size: int = 10) -> int:
        ...
        for vector in vectors:
            ...
            if len(current_batch) >= batch_size:
                try:
                    ...
                    logger.info(f"Upserted batch of {len(current_batch)} vectors. Total: {total_upserted}")
                except Exception as e:
        ...
        if current_batch:
            try:
                self.index.upsert(vectors=current_batch)
                total_upserted += len(current_batch)
                logger.info(f"Upserted final batch of {len(current_batch)} vectors. Total: {total_upserted}")
            except Exception as e:
